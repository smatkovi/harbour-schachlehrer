...
Item {
    ...
    property real squareSide: side / 8
    ...
    property variant squares: teacher.squares
    property variant targets: teacher.legalTargets
    property int selected: teacher.selectedSquare
    property bool flipped: teacher.flipped
    ...
    property int lastFrom: squareByName(("" + teacher.lastMove).substring(0, 2))
    property int lastTo: squareByName(("" + teacher.lastMove).substring(2, 4))
    ...
    Rectangle {
        ...
        GridView {
            ...
            delegate: Item {
                ...
                property int sq: board.squareOfCell(model.cell)
                property bool darkSquare: (board.fileOf(sq) + board.rankOf(sq)) % 2 === 0
                property string code: board.pieceAt(sq)
                property bool isSelected: board.selected === sq
                property bool isMoveTarget: board.showLegalTargets
                                                     && board.selected >= 0
                                                     && board.isTarget(sq)
                property bool isLastMove: board.showLastMove
