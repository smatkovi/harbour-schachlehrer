...
Item {
    ...
    property variant check: teacher.blunderCheck
    property bool active: check !== undefined && check !== null
                                   && check.active === true
    property variant moves: (check && check.moves) ? check.moves : []

    // The indices the learner has marked as dangerous.
    property variant marked: []
